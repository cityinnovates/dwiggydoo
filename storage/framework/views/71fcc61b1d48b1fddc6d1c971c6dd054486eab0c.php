					...
					<div class="row">
								...
								<?php   if($product->file_path!=''){ 
                            		$fgf=explode('.',$product->file_path); 
					      			if($fgf[2]=='jpg'||$fgf[2]=='jpeg'||$fgf[2]=='png'||$fgf[2]=='svg'||$fgf[2]=='jpg'||$fgf[2]=='gif') {  ?>
				    							<img src="<?php echo e($product->file_path); ?>" style="height: 200px;" >
								<?php  } else {   ?>
					       			<video controls>
						...
						<div class="col-lg-12 mb-4">
							<div class="bl_Convers_cnt">
								...
								<strong class="f-18 f-sbold color-00 mb-2"><?php echo e($product->name); ?></strong>
								...
								<?php	
									...
									if($requestsv>0){   ?>
										...
										<a><button value="<?php echo e($product->id); ?>" class="gotham-medium f-20 color-f3 bg-trsprnt bdr-none bdr-rdius6 py-2"><i class="fas fa-user-plus mr-2 color-f3"></i> Connected</button></a>
									 
									
									<?php } else if($requests>0) {   ?>

										<a><button  style="color: #128cf7;" value="<?php echo e($product->id); ?>" class="gotham-medium f-20 color-f3 bg-trsprnt bdr-none bdr-rdius6 py-2"><i  style="color: #128cf7;" class="fas fa-user-plus mr-2 color-f3"></i> Request Sent</button></a>

									<?php  } else { $brddd=@$_GET['breed']; ?>
																	  
								  	<a <?php if(isset(Auth::user()->id)){?> href="https://dwiggydoo.com/send_requests/<?php echo e($product->id); ?>?breed=<?php echo e($brddd); ?>"  <?php  } else {  ?> data-toggle="modal" data-target="#staticBackdrop" <?php  }  ?>  ><button value="<?php echo e($product->id); ?>" class="gotham-medium f-20 color-f3 bg-trsprnt bdr-none bdr-rdius6 py-2"><i class="fas fa-user-plus mr-2 color-f3"></i> Send Request</button></a>
								  	<?php  }  ?>
							</div>
						</div>
					</div>
...
<?php echo $__env->make('frontend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/outstand/public_html/dwiggydoo/resources/views/frontend/all-connections.blade.php ENDPATH**/ ?>