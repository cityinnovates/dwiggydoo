	...
	<div class="row align-items-center">
        ...
        @if($type != 'Seller')
    		<div class="col-md-6 text-md-right">
    			<a href="{{ route('ecom_products.create') }}" class="btn btn-circle btn-info">
            ...
            <tbody>
                @foreach($products as $key => $product)
                    <tr>
						...
						<td class="text-right">
							<a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('ecom_products.admin.edit', ['id'=>$product->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
							   <i class="las la-edit"></i>
						   </a>
							<a class="btn btn-soft-success btn-icon btn-circle btn-sm" href="{{route('ecom_products.duplicate', ['id'=>$product->id, 'type'=>$type]  )}}" title="{{ translate('Duplicate') }}">
							   <i class="las la-copy"></i>
						   </a>
                           <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('ecom_products.destroy', $product->id)}}" title="{{ translate('Delete') }}">
...
@section('script')
    <script type="text/javascript">
        ...
        function update_todays_deal(el){
            ...
            $.post('{{ route('ecom_products.todays_deal') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
        ...
        function update_published(el){
            ...
            $.post('{{ route('ecom_products.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
        ...
        function update_featured(el){
            ...
            $.post('{{ route('ecom_products.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
