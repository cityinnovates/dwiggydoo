...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="row align-items-center">
		<div class="col">
			<h1 class="h3">{{ translate('Edit Profession') }}</h1>
		</div>
	</div>
</div>
<div class="card">

	<form class="p-4" action="{{ route('profession.update') }}" method="POST" enctype="multipart/form-data">
		@csrf
	  ...
	  <h6 class="fw-600 mb-0">{{ translate('Profession Content') }}</h6>
		...
		<div class="card-body">
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input type="text" class="form-control" placeholder="Title" name="title" value="{{ $profession->title }}" required>
				</div>
			</div>
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<select name="category" class="form-control">
						...
						<option value="{{$value->id}}" <?= $value->id == $profession->category ? 'selected' : '' ;?>>{{$value->name}}</option>
						@endforeach
					</select>
				</div>
			</div>
			<div class="form-group row">
					...
					<div class="col-sm-10">
						<div class="input-group " data-toggle="aizuploader" data-type="image">
								...
								<input type="hidden" name="photo" value="{{$profession->photo}}" class="selected-files">
						</div>
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<textarea
						...
						name="description"
						required
					>@php echo $profession->description @endphp</textarea>
				</div>
			</div>
